-- profitRate를 profit_rate로 변경하고 numeric 타입으로 변경
DO $$ 
BEGIN
  -- profitRate 컬럼이 있으면 profit_rate로 이름 변경
  IF EXISTS (SELECT 1 FROM information_schema.columns 
             WHERE table_schema = 'public' AND table_name = 'assets' AND column_name = 'profitRate') THEN
    ALTER TABLE "assets" RENAME COLUMN "profitRate" TO "profit_rate";
    RAISE NOTICE 'profitRate 컬럼을 profit_rate로 이름 변경 완료';
  END IF;
  
  -- profit_rate 컬럼의 타입을 numeric으로 변경
  IF EXISTS (SELECT 1 FROM information_schema.columns 
             WHERE table_schema = 'public' AND table_name = 'assets' AND column_name = 'profit_rate' 
             AND data_type != 'numeric') THEN
    ALTER TABLE "assets" 
      ALTER COLUMN "profit_rate" TYPE numeric(10, 2) USING profit_rate::numeric(10, 2);
    RAISE NOTICE 'profit_rate 컬럼 타입을 numeric(10,2)로 변경 완료';
  END IF;
END $$;
